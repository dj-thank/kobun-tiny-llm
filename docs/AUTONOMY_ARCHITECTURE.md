...
これにより、script ごとにバラバラの dict を作るのではなく、同じ意味を同じ形で扱えるようにします。型付き contract は、自律運用を「なんとなくの判断」ではなく、検査可能な状態遷移として扱うための土台です。
...
LLM は自律レイヤーの判断材料を整理したり、独立レビューでリスクを指摘したりできます。ただし、LLM の文章は学習コーパスにも、checkpoint-bound metric の代替にも、release gate の抜け道にもなりません。

この境界を保つことで、LLM を「生成するモデル」と「公開安全性を検査する補助者」の両方に使う場合でも、責務が混ざらないようにしています。
